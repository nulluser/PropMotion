...
void Adafruit_PWMServoDriver::setPWMArray(uint16_t on_times[], uint8_t count)
{
  // Send data as four banks of 4. I2C transaction limit is 32 bytes / transaction

  bool done = false;
  int index = 0;
  
  for (int bank = 0; bank < 4 && !done; bank++)
  {
    _i2c->beginTransmission(_i2caddr);
    _i2c->write(PCA9685_LED0_ON_L + 4 * (bank * 4));
    ...
    for (int i = 0; i < 4 && !done; i++)
    {
      _i2c->write(0);
      _i2c->write(0);
      _i2c->write(on_times[index]);
      _i2c->write(on_times[index] >> 8);
