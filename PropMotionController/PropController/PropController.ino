...
void send_serial_sync()
{
  Serial.write("x");
  last_serial_sync = millis();
}

// See if we have any serial data, or send sync if timeout
void check_serial()
{
  // Send keep alive pulse to trigger next transmission
  if (millis() - last_serial_sync > 1000)
  {
    serial_state = SS_WAIT1;
    send_serial_sync();

    // Wait for start of packet or timeout. Prevents packet from coming in during pwm update
    while(millis() - last_serial_sync < 1000 && !Serial.available());
  }
